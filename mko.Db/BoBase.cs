...
namespace mkoIt.Db
{
        ...
        where TEntityView : class, new()
    {
        ...
        /// </summary>
        protected List<SortColumnDef> SortJob = new List<SortColumnDef>();

        public void SortJobDefine(IEnumerable<SortColumnDef> sortJobDef)
        {
            SortJob.Clear();
        ...
        /// </summary>
        /// <returns></returns>
        public abstract TEntityView CreateView(); 
        ...
        /// </summary>
        /// <returns></returns>
        public TEntityView CreateDummy()
        {
            return CreateDummyOn ? CreateView() : null;
        }        
        ...
        public IQueryable<TEntity> ApplyFilter()
        {
            try
            {
                return filter(EntityCollection);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Wendet eine Aktion auf alle Elemente einer gefilterten Menge an
        /// </summary>
        /// <param name="processView">Anzuwendende Aktion(z.B. Lambda- Ausdruck)</param>
        public void ForEachSelected(Action<TEntityView> processView)
        {
            try
            {
                if (Any())
                {
                    foreach (var entity in filter(EntityCollection))
                    {
        ...
        public abstract void AddEntityToEntityCollection(TEntity entity);
